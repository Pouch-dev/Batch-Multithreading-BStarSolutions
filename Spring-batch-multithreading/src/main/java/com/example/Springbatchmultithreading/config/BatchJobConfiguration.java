...
//chua tao thanh class abstract
public abstract class BatchJobConfiguration {

    @Value("${reader.input.file}") protected String inputFile;
    @Value("${async.thread.max.pool}") protected String maxThread;
    @Value("${async.thread.core.pool}") protected String core;
    @Value("${async.thread.queue}") protected String queue;
    @Value("${job.chunk.size}") protected String chunk;
    @Autowired
